...
const day = (activeDays, props, i) => `<td class="sdp-day-body${props.nextMonth ? ' sdp-day-next-month sdp-day-disabled' : ''}${props.previousMonth ? ' sdp-day-prev-month sdp-day-disabled' : ''}${props.active ? ' sdp-day-selected' : ''}"><button class="sdp-day-btn" role="button" data-model-index="${i}" aria-label="${dayNames[props.date.getDay()]}, ${monthNames[props.date.getMonth()]} ${props.date.getDate()}, ${props.date.getFullYear()}"${props.previousMonth || props.nextMonth ? " disabled" : ""}>${props.number}</button></td>`;
